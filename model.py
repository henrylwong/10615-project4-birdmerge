...
class AttriVAE_V2(nn.Module):
    def __init__(self, image_channels, hidden_dim, latent_dim, encoder_channels, decoder_channels):
        super(AttriVAE_V2, self).__init__()
        self.image_channels = image_channels
        self.latent_dim = latent_dim
        self.hidden_dim = hidden_dim
        self.encoder_channels = encoder_channels
        ...
        self.encoder = nn.Sequential(nn.Conv2d(in_channels = image_channels, out_channels = self.encoder_channels[0], kernel_size=3, stride=2, padding=1),
                                     ...
                                     nn.Conv2d(in_channels = self.encoder_channels[1], out_channels = self.encoder_channels[2], kernel_size=3, stride=1, padding=1),
                                     nn.BatchNorm2d(self.encoder_channels[2]),
                                     nn.LeakyReLU(),
                                     ...
                                     nn.Conv2d(in_channels = self.encoder_channels[3], out_channels = self.encoder_channels[4], kernel_size=3, stride=2, padding=1),
                                     nn.BatchNorm2d(self.encoder_channels[4]),
                                     nn.LeakyReLU(),
                                     
                                     nn.Conv2d(in_channels = self.encoder_channels[4], out_channels = self.encoder_channels[5], kernel_size=3, stride=1, padding=1),
                                     nn.BatchNorm2d(self.encoder_channels[5]),
                                     nn.LeakyReLU(),)
        
        self.fc_encoder = nn.Sequential(nn.Flatten(), 
                                        nn.Linear(8 * 8 * self.encoder_channels[5], 8 * 8 * self.encoder_channels[5] // 2),
                                        nn.Linear(8 * 8 * self.encoder_channels[5] // 2, self.hidden_dim)) # @henry: Attri-VAE has intermediate layer here
        ...
        self.decoder = nn.Sequential(nn.Conv2d(in_channels = self.encoder_channels[5], out_channels=self.decoder_channels[0], kernel_size=3, stride=1, padding=1),
                                     nn.BatchNorm2d(self.decoder_channels[0]),
                                     nn.LeakyReLU(),
                                     ...
                                     nn.ConvTranspose2d(in_channels = self.decoder_channels[2], out_channels=self.decoder_channels[3], kernel_size=3, stride=1, padding=1, output_padding=1),
                                     nn.BatchNorm2d(self.decoder_channels[3]),
                                     nn.LeakyReLU(),
                                     ...
                                     nn.ConvTranspose2d(in_channels = self.decoder_channels[4], out_channels=self.decoder_channels[5], kernel_size=3, stride=2, padding=1, output_padding=1),
                                     nn.BatchNorm2d(self.decoder_channels[5]),
                                     nn.LeakyReLU(),
                                     
                                     nn.Conv2d(in_channels = self.decoder_channels[5], out_channels=image_channels, kernel_size=3, stride=1, padding=1),
                                     nn.BatchNorm2d(image_channels),
                                     nn.LeakyReLU(),
...
if __name__ == "__main__":
    model = AttriVAE(3, 96, 64, (8, 16, 32, 64, 3, 2), (64, 32, 16, 8, 4, 3, 2))
